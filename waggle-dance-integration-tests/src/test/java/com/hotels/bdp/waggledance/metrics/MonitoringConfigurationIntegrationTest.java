...
public class MonitoringConfigurationIntegrationTest {
  ...
  public void graphiteReporterAllMetricsAreLoggedWhenPollNotCalled() throws Exception {
    ...
    // Using a very long poll interval so it won't actually poll in the test, this is because we
    // want to test that the
    // GraphiteReporter (ScheduledReporter) report() method is called to flush the remaining metrics
    // before closing.
    ...
    GraphiteMeterRegistry graphiteMeterRegistry =
        monitoringConfiguration.graphiteMeterRegistry(graphiteConfiguration);
