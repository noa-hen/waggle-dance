...
public final class GrammarUtils {
  ...
  private static String DEFAULT_CAT_NAME =
      StringUtils.join(
          String.valueOf(CATALOG_DB_THRIFT_NAME_MARKER),
   ...
   *
   * <p>This implementation is using {@link
   * org.apache.hadoop.hive.metastore.ObjectStore#getDatabases(String)} as reference for pattern
   * matching.
   *
   * <p>To learn more about Hive DDL patterns refer to <a
   * href="https://cwiki.apache.org/confluence/display/Hive/LanguageManual+DDL#LanguageManualDDL-Show">Language
   ...
   */
  public static Map<String, String> selectMatchingPrefixes(
      Set<String> prefixes, String dbPatterns) {
    ...
    for (Entry<String, Set<String>> prefixPatternEntry : prefixPatterns.entrySet()) {
      matchingPrefixes.put(
          prefixPatternEntry.getKey(), OR_JOINER.join(prefixPatternEntry.getValue()));
    }
    return matchingPrefixes;
  }
  ...
  public static String removeCatName(String dbPatterns) {
    ...
    if (StringUtils.startsWithIgnoreCase(
        dbPatterns, String.valueOf(CATALOG_DB_THRIFT_NAME_MARKER))) {
      dbPatterns =
          StringUtils.removeIgnoreCase(dbPatterns, String.valueOf(CATALOG_DB_THRIFT_NAME_MARKER));
    }
