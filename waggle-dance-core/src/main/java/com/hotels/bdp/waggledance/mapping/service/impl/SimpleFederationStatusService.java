...
public class SimpleFederationStatusService implements FederationStatusService {
   ...
   *
   * <p>Note: we expect this service should have a low usage so creating a client on each request is
   * fine. If the usage increases impacting the performance of the server then caching or an
   * alternative solution could be adopted.
   *
   * @param abstractMetaStore the metastore to check
   * @return {@code MetaStoreStatus.AVAILABLE} if the service can successfully connect to the
   *     metastore. Otherwise, returns {@code MetaStoreStatus.UNAVAILABLE}
   */
  @Override
