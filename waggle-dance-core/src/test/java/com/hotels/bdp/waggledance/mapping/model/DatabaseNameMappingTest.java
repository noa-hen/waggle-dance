...
public class DatabaseNameMappingTest {
  ...
  public void mapNames() throws Exception {
    ...
    DatabaseNameMapping databaseNameMapping =
        new DatabaseNameMapping(metaStoreMapping, databaseNameMap);
    assertThat(databaseNameMapping.transformInboundDatabaseName("local_name"), is("remote_name"));
    assertThat(
        databaseNameMapping.transformInboundDatabaseName("pre_remote_name"), is("remote_name"));
    assertThat(
        databaseNameMapping.transformInboundDatabaseName("pre_local_name"), is("remote_name"));
    assertThat(
        databaseNameMapping.transformOutboundDatabaseName("remote_name"), is("pre_remote_name"));
    List<String> expected = Lists.newArrayList("pre_remote_name", "pre_local_name");
    assertThat(
        databaseNameMapping.transformOutboundDatabaseNameMultiple("remote_name"), is(expected));
  }
