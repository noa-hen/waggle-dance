...
public class YamlFederatedMetaStoreStorageTest {
  ...
  public void loadFederation_empty() throws Exception {
    ...
    YamlFederatedMetaStoreStorage storage =
        new YamlFederatedMetaStoreStorage(file.toURI().toString(), configuration);
  ...
  public void loadFederationMultipleFederations() throws Exception {
    ...
    assertThat(storage.getAll().size(), is(3));
    assertThat(
        storage.getAll().get(0), is(newPrimaryInstance("hcom_3", "thrift://localhost:39083")));
    assertThat(
        storage.getAll().get(1), is(newFederatedInstance("hcom_1", "thrift://localhost:19083")));
  ...
  public void loadFederationMappedDatabasesAndTables() throws Exception {
    ...
    assertThat(
        storage.getAll().get(0), is(newPrimaryInstance("hcom_2", "thrift://localhost:39083")));
    ...
    assertThat(
        federatedMetastore.getMappedTables().get(0).getMappedTables(),
        is(Lists.newArrayList("tbl1")));
    assertThat(federatedMetastore.getMappedTables().get(1).getDatabase(), is("db2"));
    assertThat(
        federatedMetastore.getMappedTables().get(1).getMappedTables(),
        is(Lists.newArrayList("tbl2")));
  }
  ...
  public void saveFederationWriteFederations() throws Exception {
    ...
    FederatedMetaStore newFederatedInstance =
        newFederatedInstance("hcom_2", "thrift://localhost:29083");
    ...
    newFederatedInstance.setConfigurationProperties(
        Collections.singletonMap("hive.metastore.kerberos.principal", "hive/_HOST@REALM"));
  ...
  public void insertWithSameNameFails() {
    ...
    storage.insert(primary);
    FederatedMetaStore federatedWithSameName =
        AbstractMetaStore.newFederatedInstance(primary.getName(), "uris");
  ...
  public void savePrimaryWriteFederations() throws Exception {
    ...
    primaryMetaStore.setConfigurationProperties(
        Collections.singletonMap("hive.metastore.kerberos.principal", "hive/_HOST@REALM"));
