...
public class TunnelingMetaStoreClientFactoryTest {
  ...
  private @Captor ArgumentCaptor<TunnelableSupplier<CloseableThriftHiveMetastoreIface>>
      tunnelableSupplierCaptor;
  ...
  public void init() {
    ...
    waggleDanceConfigurationProperties.put(
        ConfVars.METASTORE_CLIENT_CONNECT_RETRY_DELAY.varname, "5");
    waggleDanceConfigurationProperties.put(
        ConfVars.METASTORE_USE_THRIFT_FRAMED_TRANSPORT.varname, "true");
    waggleDanceConfigurationProperties.put(
        ConfVars.METASTORE_USE_THRIFT_COMPACT_PROTOCOL.varname, "false");

    when(localHiveConfFactory.newInstance(
            any(String.class), any(Integer.class), any(HiveConf.class)))
    ...
    when(hiveMetaStoreClientSupplierFactory.newInstance(
            localHiveConf, NAME, RECONNECTION_RETRIES, CONNECTION_TIMEOUT))
        .thenReturn(hiveMetaStoreClientSupplier);

    tunnelingMetaStoreClientFactory =
        new TunnelingMetaStoreClientFactory(
            tunnelableFactorySupplier, localHiveConfFactory, hiveMetaStoreClientSupplierFactory);
  }

  @Test
  public void newInstance() {
    tunnelingMetaStoreClientFactory.newInstance(
        METASTORE_URI,
        metastoreTunnel,
        NAME,
        RECONNECTION_RETRIES,
        CONNECTION_TIMEOUT,
        waggleDanceConfigurationProperties);
    verify(tunnelableFactory)
        .wrap(
            tunnelableSupplierCaptor.capture(),
            eq(tunnelingMetaStoreClientFactory.METHOD_CHECKER),
            eq(metastoreTunnel.getLocalhost()),
            anyInt(),
            eq(METASTORE_HOST),
            eq(METASTORE_PORT));
    TunnelableSupplier<CloseableThriftHiveMetastoreIface> tunnelable =
        tunnelableSupplierCaptor.getValue();
  ...
  public void newInstanceNullConfigurationProperties() {
    tunnelingMetaStoreClientFactory.newInstance(
        METASTORE_URI, metastoreTunnel, NAME, RECONNECTION_RETRIES, CONNECTION_TIMEOUT, null);
  }
  ...
  public void localHiveConfigUsesCorrectParameters() {
    ...
    verify(localHiveConfFactory)
        .newInstance(localHostCaptor.capture(), anyInt(), hiveConfCaptor.capture());
